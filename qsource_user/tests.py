...
# The UserDataTests contains all test functions for testing all the data 
...
class UserDataTests(TestCase):
    ...
    def test_cannot_answer_twice(self):
        # Check behavior when duplicate answer attempts are made. QSource should
        # block a second vote, even if the other choice is selected.
        new_user = User.objects.create_user('john', 'lennon@thebeatles.com', 'johnpassword')
        new_data = UserData.objects.create(user = new_user)
        newQ = create_question("Anybody out there?", "yes", "no")
        new_data.answer(newQ, 0)
        new_data.answer(newQ, 1)
        # Make sure the correct vote, and only the correct vote, has been
        # registered.
        self.assertEqual(newQ.ans1_votes, 1)
        self.assertEqual(newQ.ans2_votes, 0)
        self.assertEqual(newQ.votes, 1)
        # Make sure the database hasn't stored an extra QuestionsAnswered
        # objects.
...
# Simluate authentication of a new user to allow UserData to interact with the
# view.
...
class UserViewTests(TestCase):
    # Test a submission to the menu with no changed options.
    ...
    # Test a submission to the menu with a single changed option.
    ...
    # Test a submission to the menu with all options changed.
    ...
    # Test to see if the showRecent = True feed setting works.
    ...
    # Test to see if the showRecent = False setting works (sort by most votes).
    ...
    # Test to see if showMyQuestions = True works. Only questions asked by this
    # should show up.
    def test_my(self):
        c = Client()
        new_user = new_user_login(c)
    ...
    # Test to see if showMyQuestions = False works (all applicable questions
    # shown).
    def test_all(self):
        c = Client()
        new_user = new_user_login(c)
